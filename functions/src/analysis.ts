...
const toProgramme = (value: unknown): string => {
  ...
  if (normalized === "MTLDK" || normalized === "kpmd-2") return "MTLDK";
...
const getProgrammeQueryValues = (programme: unknown): string[] => {
  ...
  if (normalized === "MTLDK") return ["MTLDK"];
  return [normalized];
};
...
const canViewAllProgrammes = (user: any): boolean => {
  return getAllowedProgrammes(user).length >= PROGRAMME_OPTIONS.length;
};
...
const createLivestockAnalytics = async (
  ...
  for (const farmer of filteredFarmers) {
    ...
    if (farmerDate && quarterCountingCutoff && farmerDate.getFullYear() === analysisYear && farmerDate <= quarterCountingCutoff) {
