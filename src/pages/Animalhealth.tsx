...
const AnimalHealthPage = () => {
  ...
  const fetchActivities = useCallback(async () => {
    try {
      ...
      if (snapshot.exists()) {
        ...
        writeCachedValue(animalHealthCacheKey, sorted);
      } else {
        setActivities([]);
        removeCachedValue(animalHealthCacheKey);
      }
  ...
  const activitiesByProgramme = useMemo(
    () => ({
      ...
      MTLDK: filteredActivities.filter((a) => a.programme === "MTLDK"),
    }),
    [filteredActivities],
  );
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <Card className="bg-white shadow-sm border-slate-200">
            <CardHeader className="flex flex-row items-center justify-between pb-2 pt-4 px-4">
              <CardTitle className="text-sm font-medium text-slate-600">Regional Coverage</CardTitle>
